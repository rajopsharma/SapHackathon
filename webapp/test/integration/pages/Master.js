			...
			onTheMasterPage : {
				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("GEN_ID") === oObject.GEN_ID;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].GEN_ID;
										} else {
											sCurrentId = aItemsNotInTheList[0].GEN_ID;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("KanbanTickets", {
											GEN_ID : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("GEN_ID"),
							title: oBindingContext.getProperty("WORKSTAT")
						};
					}
				},
				...
				assertions : {
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("WORKSTAT");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all KanbanTickets to be able to check for the total amount
